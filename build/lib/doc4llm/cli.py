...
class CLIConfig:
    ...
    def load(config_file="doc4llm/config/config.json"):
        ...
        if not os.path.exists(config_file):
            print(
                f"{Fore.YELLOW}=== 配置文件 {config_file} 不存在，使用默认配置 ==={Style.RESET_ALL}"
            )
    ...
    def _read_config(config_file):
        ...
        try:
            ...
            print(
                f"{Fore.RED}读取配置文件时出错: {type(e).__name__}: {e}{Style.RESET_ALL}"
            )
            sys.exit(1)
...
class ArgumentParser:
    ...
    def parse():
        ...
        parser.add_argument(
            "-f", dest="url_file", type=str, help="批量URL文件，每行一个URL"
        )
        parser.add_argument(
            "-config",
            dest="config_file",
            type=str,
            default="doc4llm/config/config.json",
            help="配置文件路径 (默认: doc4llm/config/config.json)",
        )
        ...
        parser.add_argument(
            "-debug", dest="debug_mode", type=int, help="调试模式 1开启 0关闭"
        )
        ...
        parser.add_argument(
            ...
            dest="url_scope_mode",
            type=int,
            help="URL扫描范围模式 0主域 1外部一次 2全放开 3白名单模式",
        )
        parser.add_argument(
            "-danger",
            dest="danger_filter_enabled",
            type=int,
            default=1,
            help="危险接口过滤 1开启 0关闭 (默认: 1)",
        )
        parser.add_argument(
            "-fuzz",
            dest="fuzz",
            type=int,
            default=0,
            help="是否启用自定义URL拼接参数 1启用 0关闭 (默认: 0)",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="爬取模式 0=仅爬取CSV 1=抓取文档内容 2=抓取锚点链接 3=内容+锚点 4=单页面爬取(不递归) (默认: 0)",
        )
        parser.add_argument(
            "-force-scan",
            dest="force_scan",
            type=int,
            default=0,
            help="强制启动URL扫描器 (mode 1/2/3时有效，1=强制扫描刷新CSV，0=CSV不为空则跳过扫描) (默认: 0)",
        )
        parser.add_argument(
            "-doc-dir", dest="doc_dir", type=str, help="文档输出目录路径"
        )
        parser.add_argument(
            "-doc-name", dest="doc_name", type=str, help="文档名称（覆盖自动检测）"
        )
        parser.add_argument(
            "-doc-version",
            dest="doc_version",
            type=str,
            help="文档版本号 (默认: latest)",
        )
        parser.add_argument(
            "-doc-depth",
            dest="doc_max_depth",
    ...
    def print_program_info():
        ...
        print(
            f"{Fore.YELLOW}=== BY: {ArgumentParser.AUTHOR}  GitHub: {ArgumentParser.GITHUB_URL}"
        )
        print(f"{Fore.YELLOW}=== 重复的URL不会重复扫描, 结果返回相同的URL不会重复展示")
        print(f"{Fore.CYAN}=== 所有输出将同时记录到 results/output.out 文件中")
        print(
            f"{Fore.CYAN}=== 扫描开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def print_usage_examples():
        ...
        examples = [
            ...
            (
                "文档爬取模式",
                "python cli.py -u https://example.com -mode 3 -force-scan 1",
            ),
...
class ArgumentValidator:
    ...
    def validate(args, config_data):
        ...
        force_scan = (
            args_force_scan
            if args_force_scan is not None
            else config_data.get("force_scan", 0)
        )
        ...
        if (
            needs_scan_target
            and not args.start_url
            and not args.url_file
            and not config_start_url
        ):
            print(
                f"{Fore.RED}错误：必须通过 -u 或 -f 至少指定一个扫描目标！{Style.RESET_ALL}"
            )
            if config_start_url:
                print(
                    f"{Fore.YELLOW}提示：config.json 中已配置 start_url: {config_start_url}{Style.RESET_ALL}"
                )
            if mode in [1, 2, 3] and force_scan == 0:
                print(
                    f"{Fore.YELLOW}提示：mode {mode} 且不使用 -force-scan 时，如果CSV文件已存在且有内容，可以不指定 -u 或 -f{Style.RESET_ALL}"
                )
            print(
                f"{Fore.YELLOW}提示：使用 -u <URL> 或 -f <文件> 指定扫描目标{Style.RESET_ALL}"
            )
...
class ConfigBuilder:
    ...
    def _load_headers(headers_path):
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}警告: 无法加载 headers 文件 {headers_path}: {e}{Style.RESET_ALL}"
            )
            return None
    ...
    def build(args, config_data):
        ...
        def get_value(key, default=None):
            return (
                ...
                else config_data.get(key, default)
            )
        ...
        headers = (
            ConfigBuilder._load_headers(headers_path)
            if headers_path
...
class OutputManager:
    ...
    def cleanup(self):
        ...
        if self.output_logger:
            ...
            self.output_logger.close()
            print(
                f"{Fore.GREEN}输出日志已保存到: {self.output_logger.log_file}{Style.RESET_ALL}"
            )
        print(
            f"{Fore.GREEN}扫描结束时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Style.RESET_ALL}"
        )
...
class ScannerRunner:
    ...
    def scan_multiple(config, url_file, get_config_value):
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}从文件读取到 {len(urls)} 个URL，开始批量扫描...{Style.RESET_ALL}"
            )
            ...
            for i, url in enumerate(urls, 1):
                try:
                    print(
                        f"{Fore.CYAN}[{i}/{len(urls)}] 开始扫描: {url}{Style.RESET_ALL}"
                    )
                    url_config = ConfigBuilder.build(
                        type("Args", (), {"start_url": url}).__dict__,
                        {
                            k: get_config_value(k) for k in ["start_url"] if False
                        },  # 使用原始配置
                    )
                    # 复制配置但修改start_url
                    ...
                    if hasattr(scanner, "config"):
                        for danger_url in URLMatcher.danger_api_filtered:
                            danger_types = [
                                api
                                for api in scanner.config.danger_api_list
                                if api in danger_url and not danger_url.endswith(".js")
                            ]
                            if danger_types:
                    ...
                    print(
                        f"{Fore.RED}扫描URL {url} 时出错: {type(e).__name__}: {e}{Style.RESET_ALL}"
                    )
                    continue
            ...
            if all_results or all_external_results or all_danger_results:
                ...
                if all_external_results:
                    output_handler.append_results(
                        all_external_results, batch_summary_file
                    )
                if all_danger_results:
                    output_handler.append_results(
                        all_danger_results, batch_summary_file
                    )
                print(
                    f"{Fore.GREEN}所有扫描结果（含外链/危险接口）已汇总到: {batch_summary_file}{Style.RESET_ALL}"
                )
            else:
                print(
                    f"{Fore.YELLOW}未收集到任何扫描结果，未生成汇总文件。{Style.RESET_ALL}"
                )
        except Exception as e:
            print(
                f"{Fore.RED}处理URL文件时出错: {type(e).__name__}: {e}{Style.RESET_ALL}"
            )
            sys.exit(1)
...
class ModeExecutor:
    ...
    def execute_mode_0(self):
        ...
        if self.args.start_url:
            ...
            ScannerRunner.scan_multiple(
                self.config, self.args.url_file, lambda k: getattr(self.args, k, None)
            )
    ...
    def execute_mode_1(self):
        ...
        if csv_exists and self.config.force_scan == 0:
            print(
                f"{Fore.YELLOW}=== 检测到CSV文件已存在且有内容，跳过URL扫描 ==={Style.RESET_ALL}"
            )
            print(
                f"{Fore.YELLOW}=== 使用 -force-scan 1 可强制刷新CSV文件 ==={Style.RESET_ALL}"
            )
        else:
        ...
        if not self.config.start_url and csv_exists:
            ...
            print(
                f"{Fore.CYAN}=== 从CSV文件读取起始URL: {self.config.start_url} ==={Style.RESET_ALL}"
            )
    ...
    def execute_mode_2(self):
        ...
        if not self.config.doc_name and self.config.start_url:
            ...
            print(
                f"{Fore.CYAN}=== 从起始URL自动设置文档名称: {self.config.doc_name} ==={Style.RESET_ALL}"
            )
    ...
    def execute_mode_4(self):
        ...
        if not self.config.start_url:
            if self.args.start_url:
                ...
                print(
                    f"{Fore.RED}错误：Mode 4 必须指定 -u 参数或 config.start_url{Style.RESET_ALL}"
                )
                sys.exit(1)

        print(
            f"{Fore.CYAN}=== 爬取单个页面: {self.config.start_url} ==={Style.RESET_ALL}"
        )
    ...
    def _print_doc_mode_info(self, mode_name):
        ...
        print(
            f"{Fore.CYAN}=== 文档输出目录: {self.config.doc_dir}/{doc_name_display}@{self.config.doc_version}{Style.RESET_ALL}"
        )
        print(f"{Fore.CYAN}=== 文档版本: {self.config.doc_version}{Style.RESET_ALL}")
        print(
            f"{Fore.CYAN}=== 最大爬取深度: {self.config.doc_max_depth}{Style.RESET_ALL}"
        )
    ...
    def _execute_url_scan(self):
        ...
        if (
            not self.args.start_url
            and not self.args.url_file
            and not self.config.start_url
        ):
            print(
                f"{Fore.RED}错误：CSV文件不存在或为空，需要指定 -u 或 -f 进行URL扫描{Style.RESET_ALL}"
            )
            print(
                f"{Fore.YELLOW}提示：使用 -force-scan 1 可强制刷新CSV文件{Style.RESET_ALL}"
            )
            sys.exit(1)
        if self.args.start_url:
            ScannerRunner.scan_single(self.config, self.args.start_url)
