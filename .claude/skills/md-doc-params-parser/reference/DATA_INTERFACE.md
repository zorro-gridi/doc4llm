...
| Field | Type | Description |
|-------|------|-------------|
| `success` | boolean | Whether reading succeeded |
| `contents` | object | Map of doc_set → page contents |
| `total_line_count` | integer | Total lines of content |
| `page_count` | integer | Number of pages read |
| `requires_processing` | boolean | Whether content needs processing |
...
```json
{
  ...
  "original_line_count": 150,
  "output_line_count": 50,
...
| `processed_doc` | string | Processed markdown content |
| `compression_applied` | boolean | Whether compression was applied |
| `original_line_count` | integer | Input line count |
| `output_line_count` | integer | Output line count |
| `compression_ratio` | number | Compression ratio (output/original) |
...
| 2 → 3 | Pass through `contents`, `line_count`, `requires_processing` |
| 3 → 4 | Format `processed_doc` with compression metadata |
