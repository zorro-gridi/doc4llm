...
class Phase0aToPhase1Parser(PhaseParser):
    ...
    def parse(
        ...
        """
        Convert query-optimizer output to searcher config.
        ...
        Returns:
            Config dict for downstream phase
        """
        ...
        if output_format == "api":
            # API format: target_doc_sets as List[str]
            return {
                "query": queries,
                "target_doc_sets": doc_sets,  # List[str] for API
                "domain_nouns": domain_nouns,
                "predicate_verbs": predicate_verbs
            }
        else:
            # CLI format: doc_sets as comma-separated string
            return {
                "query": queries,
                "doc_sets": ",".join(doc_sets) if doc_sets else "",
                "domain_nouns": domain_nouns,
                "predicate_verbs": predicate_verbs
            }
...
class MultiPhaseToPhase1Parser(PhaseParser):
    ...
    def parse(
        ...
        # Merge results
        return {
            **result_0a,  # query, doc_sets/target_doc_sets, domain_nouns, predicate_verbs
...
class Phase1_5ToPhase2Parser(PhaseParser):
    ...
    def parse(
        ...
        """
        Convert LLM reranker output to reader config.
        ...
        # Build sections config for reader
        sections = []
        for result in results:
            headings = result.get("headings", [])
            sections.append({
        ...
        if output_format == "api":
            # API format: use "sections" naming (DocReaderAPI expects sections)
            return {
                "sections": sections,
            }
        else:
            # CLI format: use "page_titles" with with_metadata and format
            return {
                "page_titles": sections,
                "with_metadata": True,
                "format": "json"
            }
