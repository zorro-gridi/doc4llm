...
The fastest way to get started is to add a minimal job to your `.gitlab-ci.yml` and set your API key as a masked variable.
  ...
  1. **Add a masked CI/CD variable**
     ...
     * Add `ANTHROPIC_AUTH_TOKEN` (masked, protected as needed)
...
After adding the job and your `ANTHROPIC_AUTH_TOKEN` variable, test by running the job manually from **CI/CD** → **Pipelines** , or trigger it from an MR to let Claude propose updates in a branch and open an MR if needed.
...
If you prefer a more controlled setup or need enterprise providers:
  ...
  1. **Configure provider access** :
     * **Claude API** : Create and store `ANTHROPIC_AUTH_TOKEN` as a masked CI/CD variable
    ...
    claude:
      ...
      script:
        ...
        - >
          ...
          --allowedTools "Bash(*) Read(*) Edit(*) Write(*) mcp__gitlab"
          --debug
      # Claude Code will use ANTHROPIC_AUTH_TOKEN from CI/CD variables
...
**Never commit API keys or cloud credentials to your repository**. Always use GitLab CI/CD variables:
  ...
  * Add `ANTHROPIC_AUTH_TOKEN` as a masked variable (and protect it if needed)
...
### Claude not responding to @claude commands
  ...
  * Ensure CI/CD variables (`ANTHROPIC_AUTH_TOKEN` or cloud provider settings) are present and unmasked
...
### Authentication errors
  ...
  * **For Claude API** : Confirm `ANTHROPIC_AUTH_TOKEN` is valid and unexpired
...
Claude Code supports these commonly used inputs:
  ...
  * `ANTHROPIC_AUTH_TOKEN`: Required for the Claude API (not used for Bedrock/Vertex)
